...
int main (int argc, char *argv[])
{
    ...
    for(int i = 0; i < 3 && coordinator == false ; i++) {
        ...
        if(rank == i) {
            ...
            //pentru coordonatorii 0 si 1, acestia bor transmite/primi informatii de la coordonatorul 2
            if(rank == 0 || rank == 1) {
                    ...
                    //primeste informatii de la coordonatorul "index"(0 sau 1) prin intermediul lui 2
                    MPI_Recv(&index, 1, MPI_INT, 2, 0, MPI_COMM_WORLD, &status);
                    MPI_Recv(matrix[index], dimV, MPI_INT, 2, 0, MPI_COMM_WORLD, &status);
            }

            //coordonatorul 2 va trebui sa realizeze atat legatura intre el si ceilalti 2 coordonatori, cat si dintre 0 si 1
            if(rank == 2) {
                for(int j = 0; j <= 1; j++) {
                    //mai intai trimite mesajul 
                ...
                //ce informatie primeste de la 0, ii va transmite si lui 1
                index = 0;
                MPI_Send(&index, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);  
                MPI_Send(matrix[0], dimV, MPI_INT, 1, 0, MPI_COMM_WORLD);  

                //ce informatie primeste de la 1, ii va transmite si lui 0
                index = 1;
    ...
    if(rank == 0) {
        ...
        //mai departe coordonatorul 0 va trimite informatii despre vector catre coordonatorul 2
        MPI_Send(&N, 1, MPI_INT, 2, 0, MPI_COMM_WORLD);     
        MPI_Send(v, N, MPI_INT, 2, 0, MPI_COMM_WORLD);    
        //de asemenea va primi partea modificata din vector prin 2 (atat de la procesul 2, cat si de la procesul 1)
        ...
        //acesta primeste dimensiunea vectorului si array-ul de la coordonatorul 2
        MPI_Recv(&N, 1, MPI_INT, 2, 0, MPI_COMM_WORLD, &status);
        MPI_Recv(v, N, MPI_INT, 2, 0, MPI_COMM_WORLD, &status);


        //va trimite mai departe vectorul catre procesele worker
        //si va primi vectorul partial prelucrat de la fiecare proces worker 
        for (int j = 1; j <= matrix[rank][0]; j++) {
            MPI_Send(&N, 1, MPI_INT, matrix[rank][j], 0, MPI_COMM_WORLD);     
            MPI_Send(v, N, MPI_INT, matrix[rank][j], 0, MPI_COMM_WORLD);  
        ...
        //la final se trimite vectorul modificat catre coordonatorul 2(care acesta la randul lui ii va trimite lui 0)
        MPI_Send(v, N, MPI_INT, 2, 0, MPI_COMM_WORLD);

    } else if(rank == 2) {
        //acesta primeste dimensiunea vectorului si array-ul de la coordonatorul 0
        MPI_Recv(&N, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
        MPI_Recv(v, N, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);

        //trimite mai departe vectorul catre coordonatorul 1
        MPI_Send(&N, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);  
        MPI_Send(v, N, MPI_INT, 1, 0, MPI_COMM_WORLD);  

        //primeste partea din vector modificata de procesele worker asignate coordonatorului 1
        MPI_Recv(v, N, MPI_INT, 1, 0, MPI_COMM_WORLD, &status);

 
        //va trimite mai departe vectorul catre procesele worker
        //si va primi vectorul partial prelucrat de la fiecare proces worker de-al sau
        for (int j = 1; j <= matrix[rank][0]; j++) {
            MPI_Send(&N, 1, MPI_INT, matrix[rank][j], 0, MPI_COMM_WORLD);     
            MPI_Send(v, N, MPI_INT, matrix[rank][j], 0, MPI_COMM_WORLD);  
